...
case class HasMaxLength() extends ConstraintRule[ColumnProfile] {
  override def shouldBeApplied(profile: ColumnProfile, numRecords: Long): Boolean = {
    profile match {
      case profile: ExtendedStringColumnProfile => profile.maxLength.isDefined
      case _ => false
    }
  }

  override def candidate(profile: ColumnProfile, numRecords: Long): ConstraintSuggestion = {
    val maxLength: Double = profile.asInstanceOf[ExtendedStringColumnProfile].maxLength.get

    val constraint = maxLengthConstraint(profile.column, Check.IsOne)
