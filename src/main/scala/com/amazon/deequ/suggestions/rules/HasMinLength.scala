...
import com.amazon.deequ.profiles.{ColumnProfile, ExtendedStringColumnProfile}
...
case class HasMinLength() extends ConstraintRule[ColumnProfile] {
  override def shouldBeApplied(profile: ColumnProfile, numRecords: Long): Boolean = {
    profile match {
      case profile: ExtendedStringColumnProfile => profile.minLength.isDefined
      case _ => false
    }
  }

  override def candidate(profile: ColumnProfile, numRecords: Long): ConstraintSuggestion = {
    val minLength: Double = profile.asInstanceOf[ExtendedStringColumnProfile].minLength.get

    val constraint = minLengthConstraint(profile.column, Check.IsOne)

    ConstraintSuggestion(
      constraint,
      profile.column,
      "MinLength: " + profile.completeness.toString,
